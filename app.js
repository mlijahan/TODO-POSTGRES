...
const getTodos = async (request) => {
  try {
    const todos = await sql`SELECT item FROM todos`;
...
const getTodoById = async (request, match) => {
  ...
  try {
    const todo = await sql`SELECT item FROM todos WHERE id = ${id}`;
    if (todo.length === 0) {
      return new Response("Todo not found", { status: 404 });
    }

    const item = todo[0].item;
    return new Response(JSON.stringify({ item: todo[0].item  }), { status: 200 });
  } catch (error) {
...
const addTodo = async (request) => {
  ...
  if (!todo.item || typeof todo.item !== "string" || todo.item.trim() === "practice") {
